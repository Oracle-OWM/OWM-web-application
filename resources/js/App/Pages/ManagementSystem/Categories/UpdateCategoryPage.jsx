...
const UpdateCategoryPage = ({match}) => {
  const { loading, getCategoryById, updateCategory, message, errors, setInput, inputsState, resetAllInputs, resetAllErrors } = useContext(CategoryContext);
  const { getContent, content } = useContext(GeneralContext);

  useEffect(async () => {
    await resetAllInputs();
    await resetAllErrors();
    await getCategoryById(match.params.id)
    await getContent();
  }, []);
  ...
  async function updateHandler(e) {
    ...
    await updateCategory(match.params.id, formData);
  };
  ...
  return (<>
    <section className="w-11/12 mx-auto my-5">
      ...
      {!loading && message === 'This category is not exist' && (
        <div className="flex flex-col justify-center items-center h-screen">
          <h1 className="text-center font-extrabold text-2xl">{`This category is not exist`}</h1>
        </div>
      )}

      {(!loading && content && inputsState && (<>
        <div className="flex flex-col w-8/12 mx-auto rounded items-center bg-white">
          <h1 className="bg-blue-light p-3 w-9/12 rounded text-white z-10 -mt-5 md:text-2xl text-base">Update Category</h1>
