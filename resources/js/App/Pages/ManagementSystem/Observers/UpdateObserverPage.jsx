...
const UpdateObserverPage = ({match}) => {
  const { loading, getObserverById, updateObserver, message, errors, setInput, inputsState, resetAllInputs, resetAllErrors } = useContext(ObserverContext);
  const { getContent, content } = useContext(GeneralContext);
  
  useEffect(async () => {
    await resetAllInputs();
    await resetAllErrors();
    await getObserverById(match.params.id)
    await getContent();
  }, []);
  ...
  async function updateHandler(e) {
    ...
    await updateObserver(match.params.id, formData);    
  };
    ...
    <section className="w-11/12 mx-auto my-5">
      ...
      {(!loading && content && inputsState && (<>
        <div className="flex flex-col w-8/12 mx-auto rounded items-center bg-white">
          <h1 className="bg-blue-light p-3 w-9/12 rounded text-white z-10 -mt-5 md:text-2xl text-base">Update Observer</h1>
...
export default withRouter(UpdateObserverPage);
